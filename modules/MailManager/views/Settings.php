...
class MailManager_Settings_View extends MailManager_MainUI_View {
    ...
    file_put_contents('mailmanager_debug.log', "Saving Mailbox: Server - $server | Username - $username | Password: " . (!empty($password) ? "Provided" : "MISSING") . "\n", FILE_APPEND);
			...
			if ($connector->isConnected()) {
			    ...
			    file_put_contents('mailmanager_debug.log', "DEBUG: Attempting to Save Mailbox\n", FILE_APPEND);
				$model->save();
				file_put_contents('mailmanager_debug.log', "DEBUG: Save function executed\n", FILE_APPEND);
