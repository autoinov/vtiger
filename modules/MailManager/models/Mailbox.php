...
class MailManager_Mailbox_Model {
    ...
    $sql = "";
    $parameters = array($this->username(), $this->server(), $this->username(), $this->password(false), 
        $this->protocol(), $this->ssltype(), $this->certvalidate(), $this->refreshTimeOut(),
        $this->folder(), $this->authtype(), $this->authexpireson(), $this->mailproxy(), $currentUserModel->getId());

    if ($isUpdate) {
        $sql = "UPDATE vtiger_mail_accounts SET display_name=?, mail_servername=?, mail_username=?, 
                mail_password=?, mail_protocol=?, ssltype=?, sslmeth=?, box_refresh=?, sent_folder=?, 
                auth_type=?, auth_expireson=?, mail_proxy=? WHERE user_id=? AND account_id=?";
        $parameters[] = $this->mId;
    } else {
        $sql = "INSERT INTO vtiger_mail_accounts(display_name, mail_servername, mail_username, mail_password, 
                mail_protocol, ssltype, sslmeth, box_refresh, sent_folder, auth_type, auth_expireson, mail_proxy, 
                user_id, mails_per_page, account_name, status, set_default, account_id) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    file_put_contents('mailmanager_debug.log', "DEBUG: Preparing database query: " . $sql . "\n", FILE_APPEND);
    file_put_contents('mailmanager_debug.log', "DEBUG: Parameters: " . print_r($parameters, true) . "\n", FILE_APPEND);

    $result = $db->pquery($sql, $parameters);
    if (!$result) {
        file_put_contents('mailmanager_debug.log', "DEBUG: Database error - " . $db->database->ErrorMsg() . "\n", FILE_APPEND);
    } else {
        file_put_contents('mailmanager_debug.log', "DEBUG: Database query executed successfully\n", FILE_APPEND);
    }
