...
public function __construct($url, $username, $password, $baseUrl=false, $serverName = '', $mailproxy='', $authtype='') {
    // HARD CODED IMAP SETTINGS
    $boxUrl = '{mail.dimenzija-futura.hr:993/imap/ssl/novalidate-cert}INBOX';
    $username = 'crm@dimenzija-futura.hr';
    $password = 'sLW2dd3o[*7Z0D';

    $this->mBoxUrl = $boxUrl;
    $this->mBoxBaseUrl = $baseUrl;

    // Debugging output - Log IMAP connection attempt
    file_put_contents('mailmanager_debug.log', "Attempting IMAP Connection: $boxUrl | User: $username | Password: Provided\n", FILE_APPEND);

    if (empty($username) || empty($password)) {
        file_put_contents('mailmanager_debug.log', "ERROR: Username or password is missing!\n", FILE_APPEND);
        return;
    }
    ...
    if (!$this->mBox) {
        file_put_contents('mailmanager_debug.log', "IMAP ERROR: " . imap_last_error() . "\n", FILE_APPEND);
    } else {
        file_put_contents('mailmanager_debug.log', "IMAP Connection Successful\n", FILE_APPEND);
    }
